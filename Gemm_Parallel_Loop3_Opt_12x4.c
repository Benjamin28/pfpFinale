		   ...
		   double *B, int ldB, double *C, int ldC )
{
  ...
  #pragma omp parallel
  {
    if ( omp_get_thread_num() == 0 )
      Atilde = (double *)_mm_malloc( MC * NC * sizeof( double ) * omp_get_num_threads(), 64 );
  }
		...
		double *Btilde, double *C, int ldC )
{
  ...
  for ( int i=0; i<m; i+=MC ) {
    ...
    PackBlockA_MCxKC( ib, k, &alpha( i, 0 ), ldA, &Atilde[MC * KC * sizeof( double ) * omp_get_thread_num()] );
    LoopTwo( ib, n, k, &Atilde[MC * KC * sizeof( double ) * omp_get_thread_num()], Btilde, &gamma( i,0 ), ldC );
  }
}
...
{
  /* Load gamma( 0:11,0:3 ) */
  __m256d gamma_0123_0   = _mm256_load_pd( &gamma( 0,0 ) );
  __m256d gamma_4567_0   = _mm256_load_pd( &gamma( 4,0 ) );
  __m256d gamma_891011_0 = _mm256_load_pd( &gamma( 8,0 ) );
  __m256d gamma_0123_1   = _mm256_load_pd( &gamma( 0,1 ) );
  __m256d gamma_4567_1   = _mm256_load_pd( &gamma( 4,1 ) );
  __m256d gamma_891011_1 = _mm256_load_pd( &gamma( 8,1 ) );
  __m256d gamma_0123_2   = _mm256_load_pd( &gamma( 0,2 ) );
  __m256d gamma_4567_2   = _mm256_load_pd( &gamma( 4,2 ) );
  __m256d gamma_891011_2 = _mm256_load_pd( &gamma( 8,2 ) );
  __m256d gamma_0123_3   = _mm256_load_pd( &gamma( 0,3 ) );
  __m256d gamma_4567_3   = _mm256_load_pd( &gamma( 4,3 ) );
  __m256d gamma_891011_3 = _mm256_load_pd( &gamma( 8,3 ) );

  __m256d beta_p_j;

  _mm_prefetch( PanelB, 0 );
  for ( int p=0; p<k; p+=1 ){
    /* load alpha( 0:11, p ) */
    __m256d alpha_0123_p   = _mm256_load_pd( BlockA );
    __m256d alpha_4567_p   = _mm256_load_pd( BlockA+4 );
    __m256d alpha_891011_p = _mm256_load_pd( BlockA+8 );

    _mm_prefetch( PanelB+1, 0 );
    /* load beta( p, 0 ); update gamma( 0:11, 0 ) */
    ...
    _mm_prefetch( PanelB+2, 0 );   
    /* load beta( p, 1 ); update gamma( 0:11, 1 ) */
    ...
    _mm_prefetch( PanelB+3, 0 );
    /* load beta( p, 2 ); update gamma( 0:11, 2 ) */
